		...
		try {
			...
			for (temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					case "Bike": {
						equipment = new Bike(Integer.parseInt(id), category, title,isAvialible, Double.valueOf(rentPrice), Double.valueOf(lostPrice));
						break;
					}
					case "SnowBoard": {
					 equipment = new SnowBoard(Integer.parseInt(id), category, title,isAvialible, Double.valueOf(rentPrice), Double.valueOf(lostPrice));
					 break;
					}
					case "Skis": {
						 equipment = new Skis(Integer.parseInt(id), category, title,isAvialible, Double.valueOf(rentPrice), Double.valueOf(lostPrice));
						 break;
					}
					case "Acsessories": {
						 equipment = new Acsessories(Integer.parseInt(id), category, title,isAvialible, Double.valueOf(rentPrice), Double.valueOf(lostPrice));
						 break;
						 }
					}
					equipment.print();
